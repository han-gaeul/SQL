...
```python
gerne_list = ['액션', '드라마', '사극', '범죄', '스릴러', 'SF', '무협', '첩보', '재난']
for i in range(len(gerne_list)):
     genre = Genre()
     genre.title = gerne_list[i]
     genre.save()
```
...
```python
_id = Director.objects.get(id = 1)
print(_id.name, _id.debut, _id.country)
```
...
```
get은 쿼리에 맞는 객체 한 개만 반환
filter는 조건에 맞는 객체 여러개를 반환
```
